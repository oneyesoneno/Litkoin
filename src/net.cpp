...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Litkoin " + FormatFullVersion();
...
static const char *strTestNetDNSSeed[][2] = {
    {"litkointools.com", "testnet-seed.litkointools.com"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Litkoin is probably already running."), addrBind.ToString().c_str());
        else
