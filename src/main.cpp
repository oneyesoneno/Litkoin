...
#if defined(NDEBUG)
# error "Litkoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1e6fdd0b9d4d1d9f2f351cd2eb4bf341a9bc8fcf640374a16ea46bd98780edf1");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Litkoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Litkoin Signed Message:\n";
...
{
    // Litkoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 10 * 30; // Litkoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Litkoin: 30 Seconds.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Litkoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Litkoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1e6fdd0b9d4d1d9f2f351cd2eb4bf341a9bc8fcf640374a16ea46bd98780edf1");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "04/OKT/2017 Bermulanya Litkoin Dibentuk Meski Aneh Namun Nyata";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x05885de9bec505134ad8c5926b9d37e6119575152fb689aff75e12eb733c5717"));
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xbd }; // Litkoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LitkoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LitkoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LitkoinMiner(CWallet *pwallet)
{
    printf("LitkoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("litkoin-miner");
    ...
    try { loop {
        ...
        printf("Running LitkoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LitkoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LitkoinMiner, pwallet));
}
